...
contract SalesDatastore is Ownable {
    ...
    function encodeUSV2ExchangeID(address usv2Router_) public view onlyOwner() returns ( bytes32 usv2ExchangeID_ ) {
        require( usv2Router_.isContract() );
        return _encodeUSV2ExchangeID( usv2Router_);
    }

    // Internal function provided to optimize gas for internal calls;
    function _encodeUSV2ExchangeID(address usv2Router_) internal pure onlyOwner() returns ( bytes32 usv2ExchangeID_ ) {
        return bytes32(usv2Router_);
    }
